...
tmpFn <- function(xVect){
  return((xVect[-c(length(xVect),length(xVect)-1)]+xVect[-c(1,length(xVect))]+xVect[-c(1,2)])/3)
}
tmpFn(c(1:5,6:1))
```
...
\end{bmatrix}$$
```{r}
tmpFn_4 <- function(mat){
  mat[mat%%2 == 1] <- 2 * mat[mat%%2 == 1]
  return(mat)
}
mat_4 <- matrix(c(1,1,3,5,2,6,-2,-1,-3),nr=3,byrow=T)
tmpFn_4(mat_4)
```
...
```{r}
weekday <- function(month, day, year) {
        month <- month - 2
        if(month <= 0) {
            month <- month + 12
            year <- year - 1
        ...
        cen <- year %/% 100
        year <- year %% 100
        tmp <- floor(2.6*month - 0.2) + day + year + year %/% 4 + cen %/% 4 - 2 * cen
